...
Public Sub DeepXmlCleanup_Safe()
    
    On Error GoTo CleanFail
    
    Dim sr As Word.Range
    Dim linked As Word.Range
    Dim XML As String
    Dim storyCount As Long
    Dim lastStep As String
    Dim StoryTypeName As String
    Dim st As WdStoryType
    ...
    If MsgBox("This will rebuild the document's internal XML." & vbCrLf & _
    "It is SAFE but cannot be undone except by closing without saving." & vbCrLf & _
    vbCrLf & "Continue?", vbQuestion + vbOKCancel) <> vbOK Then
    Exit Sub
    End If
    
    '--- Begin cleanup ------------------------------------------------------------
    Application.ScreenUpdating = False
    Application.UndoRecord.StartCustomRecord "Deep XML Cleanup (Safe)"
    ...
    st = linked.StoryType
    StoryTypeName = GetStoryTypeName(st)
    
    '------------------------------------------------------------------------------
    ' SKIP FOOTNOTES & ENDNOTES
    '
    ' WHY:
    ' Word cannot safely delete and reinsert the entire Footnotes or Endnotes
    ' story as a single XML block. These stories often contain:
    '   - broken bookmarks
    '   - malformed <w:footnote> or <w:endnote> elements
    '   - corrupted rPr runs
    '   - leftover tracked-change anchors
    '   - cross-reference targets
    '
    ' When Word encounters these, InsertXML fails *inside* Word's XML parser.
    ' Word then clears Err.Number, Err.Description, and Erl - producing the
    ' classic "Erl=0 Error 0 ()" failure.
    '
    ' Therefore, these stories MUST be skipped in bulk cleanup.
    '------------------------------------------------------------------------------
    If st = wdFootnotesStory Or st = wdEndnotesStory Then
       lastStep = "Skipping story: " & StoryTypeName & _
                 " (cannot bulk-rebuild footnotes safely)"
       GoTo NextStory
    End If
    
    '--- Process safe stories -----------------------------------------------------
    lastStep = "Checking story: " & StoryTypeName & _
             " | Start=" & linked.Start & " End=" & linked.End
    
    If linked.StoryLength > 1 Then
    
       storyCount = storyCount + 1
       lastStep = "Extracting XML for " & StoryTypeName
       XML = linked.WordOpenXML
    
       lastStep = "Deleting story range " & StoryTypeName
       linked.Delete
    
       lastStep = "Reinserting XML for " & StoryTypeName & _
                 " | XML length=" & Len(XML)
       linked.InsertXML XML
...
NextStory:
    ...
    MsgBox "Deep XML cleanup complete. Stories rebuilt: " & storyCount, vbInformation
    Exit Sub
...
CleanFail:
    ...
    Debug.Print "=== XML CLEANUP FAILURE ==="
    Debug.Print "Erl=" & Erl
    Debug.Print "Error " & Err.Number & " (" & Err.Description & ")"
    Debug.Print "Last step: " & lastStep
    Debug.Print "Story type: " & StoryTypeName
    
    MsgBox "Deep XML cleanup failed." & vbCrLf & _
    "Line: " & Erl & vbCrLf & _
    "Error: " & Err.Number & " (" & Err.Description & ")" & vbCrLf & _
    "Last step: " & lastStep & vbCrLf & _
    "Story: " & StoryTypeName, vbCritical
End Sub

Private Function GetStoryTypeName(ByVal st As WdStoryType) As String
    Select Case st
        Case wdMainTextStory: GetStoryTypeName = "Body"
        Case wdPrimaryHeaderStory: GetStoryTypeName = "Primary Header"
        Case wdFirstPageHeaderStory: GetStoryTypeName = "First Page Header"
        Case wdEvenPagesHeaderStory: GetStoryTypeName = "Even Pages Header"
        Case wdPrimaryFooterStory: GetStoryTypeName = "Primary Footer"
        Case wdFirstPageFooterStory: GetStoryTypeName = "First Page Footer"
        Case wdEvenPagesFooterStory: GetStoryTypeName = "Even Pages Footer"
        Case wdTextFrameStory: GetStoryTypeName = "Textboxes"
        Case wdCommentsStory: GetStoryTypeName = "Comments"
        Case wdFootnotesStory: GetStoryTypeName = "Footnotes"
        Case wdEndnotesStory: GetStoryTypeName = "Endnotes"
        Case Else: GetStoryTypeName = "Other (" & st & ")"
    End Select
End Function

Private Function FootnotePreview(fnRange As Word.Range, Optional maxLen As Long = 80) As String
    Dim txt As String
    txt = fnRange.Text
...
Private Function ValidateFootnoteXml(fnXml As String, ByRef errMsg As String) As Boolean
    ' Try inserting the XML into a temporary range in the Footnotes story.
    ' If InsertXML fails here, we know this footnote is structurally bad,
    ' without touching the original footnote content.
    Dim testRng As Word.Range
    On Error GoTo FAIL

    Set testRng = ActiveDocument.StoryRanges(wdFootnotesStory)
    testRng.Collapse wdCollapseEnd

    testRng.InsertXML fnXml
    testRng.Delete

    ValidateFootnoteXml = True
    Exit Function

FAIL:
    errMsg = "Err " & Err.Number & " (" & Err.Description & ")"
    ValidateFootnoteXml = False
    Err.Clear
End Function

'====================================================================
' Run them in this order:
'  - DetectFootnoteCorruption_Fast
'  - IsolateCorruptFootnotes_Detailed (to study the bad ones)
'  - RebuildFootnotesSafely
' Get a full, audit-grade picture of which footnotes are structurally sound, which are not, and which were successfully normalized.
'====================================================================
Public Sub DetectFootnoteCorruption_Fast()
    Dim i As Long
    Dim fn As footnote
    Dim txt As String
    Dim XML As String
    ...
    Dim suspicious As Boolean

    Application.ScreenUpdating = False
    Application.StatusBar = "Scanning footnotes..."
    ...
    For i = 1 To ActiveDocument.Footnotes.Count
        ...
        ' --- Cheap suspicion test (fast) ---
        suspicious = False
        If InStr(txt, ChrW(8203)) > 0 Then suspicious = True ' zero-width
        If fn.Range.Fields.Count > 0 Then suspicious = True
        If fn.Range.Bookmarks.Count > 0 Then suspicious = True
        If fn.Range.Revisions.Count > 0 Then suspicious = True
        If fn.Range.Tables.Count > 0 Then suspicious = True
        If fn.Range.OMaths.Count > 0 Then suspicious = True

        ' If not suspicious, skip expensive XML test
        If Not suspicious Then GoTo NextFootnote

        ' --- Expensive XML validation only for suspicious notes ---
        On Error Resume Next
        XML = fn.Range.WordOpenXML
        If Err.Number <> 0 Then
            errMsg = "WordOpenXML error " & Err.Number & " (" & Err.Description & ")"
            Err.Clear
            GoTo BadFootnote
        End If
        On Error GoTo 0

        ok = ValidateFootnoteXml(XML, errMsg)
        If Not ok Then GoTo BadFootnote

        GoTo NextFootnote

BadFootnote:
        badList = badList & _
                  "Footnote #" & i & " | """ & FootnotePreview(fn.Range) & _
                  """ | " & errMsg & vbCrLf

NextFootnote:
        If i Mod 50 = 0 Then DoEvents ' keep Word responsive
    Next i

    Application.ScreenUpdating = True
    Application.StatusBar = False

    If Len(badList) = 0 Then
        MsgBox "No corrupt footnotes detected.", vbInformation
    Else
        Debug.Print "=== BAD FOOTNOTES DETECTED ==="
        Debug.Print badList
        MsgBox "Corrupt footnotes found. See Immediate Window.", vbCritical
    End If
End Sub

Public Sub IsolateCorruptFootnotes_Detailed()
    Dim i As Long
    Dim fn As footnote
    Dim fnXml As String
    Dim ok As Boolean
    ...
    Debug.Print "=== ISOLATE CORRUPT FOOTNOTES (DETAILED) ==="
    Debug.Print "Total footnotes: " & ActiveDocument.Footnotes.Count
    ...
    For i = 1 To ActiveDocument.Footnotes.Count
        ...
        Debug.Print "----- Footnote #" & i & " -----"
        Debug.Print "Preview: """ & FootnotePreview(fn.Range) & """"

        On Error Resume Next
        fnXml = fn.Range.WordOpenXML
        If Err.Number <> 0 Then
            errMsg = "Err " & Err.Number & " (" & Err.Description & _
                     ") while reading WordOpenXML"
            Err.Clear
        End If
        On Error GoTo 0

        If errMsg = "" Then
            ok = ValidateFootnoteXml(fnXml, errMsg)
        Else
            ok = False
        End If

        If ok Then
            Debug.Print "Status: OK (XML structure valid)"
        Else
            Debug.Print "Status: BAD (XML invalid)"
            Debug.Print "Detail: " & errMsg
        End If

    Next i

    Debug.Print "=== END ISOLATION REPORT ==="
    MsgBox "Corrupt footnote isolation complete. See Immediate Window.", vbInformation

End Sub

Public Sub RebuildFootnotesSafely()
    ...
    Dim errMsg As String
    Dim ok As Boolean
    Dim rebuiltCount As Long
    Dim skippedBad As Long
    Dim lastStep As String
    ...
    If ActiveDocument.Footnotes.Count = 0 Then
        MsgBox "No footnotes in this document.", vbInformation
        Exit Sub
    End If

    If MsgBox("This will rebuild each footnote individually using WordOpenXML." & vbCrLf & _
              "Corrupt footnotes will be detected and skipped, not deleted." & vbCrLf & _
              vbCrLf & "Continue?", vbQuestion + vbOKCancel) <> vbOK Then
        Exit Sub
    End If

    Application.ScreenUpdating = False
    Application.UndoRecord.StartCustomRecord "Rebuild Footnotes Safely"

    On Error GoTo FAIL

    Debug.Print "=== REBUILD FOOTNOTES SAFELY ==="
    Debug.Print "Total footnotes: " & ActiveDocument.Footnotes.Count
    ...
    For i = 1 To ActiveDocument.Footnotes.Count
        ...
        errMsg = ""
        ok = False
        lastStep = "Reading WordOpenXML for footnote #" & i
        ...
        If errMsg <> "" Then
            Debug.Print "Status: SKIP (cannot read XML) | " & errMsg
            skippedBad = skippedBad + 1
            GoTo NextFootnote
        End If

        ' 2) Validate XML in temp range
        lastStep = "Validating XML for footnote #" & i
        ok = ValidateFootnoteXml(fnXml, errMsg)

        If Not ok Then
            Debug.Print "Status: SKIP (XML invalid) | " & errMsg
            skippedBad = skippedBad + 1
            GoTo NextFootnote
        End If

        ' 3) Rebuild this footnote in place
        lastStep = "Rebuilding footnote #" & i

        ' Work on a duplicate range so we only touch the Note text, not the anchor.
        Dim r As Word.Range
...
NextFootnote:
    ...
    Debug.Print "=== REBUILD SUMMARY ==="
    Debug.Print "Rebuilt footnotes: " & rebuiltCount
    Debug.Print "Skipped (bad or unreadable): " & skippedBad

    MsgBox "Footnote rebuild complete." & vbCrLf & _
           "Rebuilt: " & rebuiltCount & vbCrLf & _
           "Skipped (bad/unreadable): " & skippedBad & vbCrLf & _
           "See Immediate Window for details.", vbInformation
    Exit Sub
...
FAIL:
    ...
    Debug.Print "=== FOOTNOTE REBUILD FAILURE ==="
    Debug.Print "Footnote index (approx): " & i
    Debug.Print "Last step: " & lastStep
    Debug.Print "Err " & Err.Number & " (" & Err.Description & ")"

    MsgBox "Footnote rebuild failed." & vbCrLf & _
           "Footnote index (approx): " & i & vbCrLf & _
           "Last step: " & lastStep & vbCrLf & _
           "Error: " & Err.Number & " (" & Err.Description & ")", vbCritical

End Sub

Public Sub ReportFootnoteComplexity()
    Dim i As Long
    Dim fn As footnote
    Dim txt As String
    Dim simpleCount As Long
    Dim complexCount As Long
    Dim needsRebuild As Collection
    Dim suspicious As Boolean
    Dim report As String

    Set needsRebuild = New Collection
    ...
    For i = 1 To ActiveDocument.Footnotes.Count
        ...
        ' --- FAST COMPLEXITY TEST (no XML) ---
        If fn.Range.Fields.Count > 0 Then suspicious = True
        If fn.Range.Bookmarks.Count > 0 Then suspicious = True
        If fn.Range.Revisions.Count > 0 Then suspicious = True
        If fn.Range.Tables.Count > 0 Then suspicious = True
        If fn.Range.OMaths.Count > 0 Then suspicious = True
        If InStr(txt, ChrW(8203)) > 0 Then suspicious = True ' zero-width char
        If InStr(txt, ChrW(12)) > 0 Then suspicious = True   ' form feed
        If InStr(txt, ChrW(11)) > 0 Then suspicious = True   ' vertical tab
    ...
    ' --- BUILD REPORT ---
    report = "=== FOOTNOTE COMPLEXITY REPORT ===" & vbCrLf & vbCrLf & _
             "Total footnotes: " & ActiveDocument.Footnotes.Count & vbCrLf & _
             "Simple footnotes: " & simpleCount & vbCrLf & _
             "Complex footnotes: " & complexCount & vbCrLf & vbCrLf & _
             "=== FOOTNOTES NEEDING REBUILD ===" & vbCrLf

    If needsRebuild.Count = 0 Then
        report = report & "(None)" & vbCrLf
    Else
        For i = 1 To needsRebuild.Count
            report = report & "#" & needsRebuild(i) & vbCrLf
        Next i
    End If
    Debug.Print report

    MsgBox "Footnote complexity report complete." & vbCrLf & _
           "Simple: " & simpleCount & vbCrLf & _
           "Complex: " & complexCount & vbCrLf & _
           "See Immediate Window for full list.", vbInformation
End Sub
