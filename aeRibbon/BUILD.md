...
6. **Clean up the Startup-folder copy.** Delete
   `%APPDATA%\Microsoft\Word\STARTUP\aeRibbon.dotm` now that G7/G8 are
   done — it is a disposable deployment artefact regenerated by this
   build procedure (steps 1-3 above), not something that should persist
   for day-to-day VBA dev work on the SSOT `.docm`. Leaving it installed
   causes a duplicate "Radiant Word Bible" tab the next time a dev
   `.docm` (which carries its own embedded ribbon per the dotm/docx
   split) is opened — confirmed 2026-09-19. `ThisDocument.cls`'s
   `CheckForConflictingRibbonAddIn` (added 2026-09-19) catches this and
   offers to delete it automatically if it's ever left behind anyway.
